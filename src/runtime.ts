...
export const HELP_TEXT = [
  ...
  "  mv, cp, rm, search, glob, grep, rg, reindex, import-tree",
  "",
  "examples:",
  "  ctx_cli ls ctx://alice/defaultWorkspace --cloud",
  "  ctx_cli write ctx://alice/defaultWorkspace/a.md --text \"hello\" --cloud",
  "  ctx_cli ls ./memory --local",
  "  ctx_cli read ./memory/2026-03-17.md --local",
  "  ctx_cli import-tree ./memory ctx://alice/defaultWorkspace/memory --include '*.md' --cloud",
].join("\n");
...
export class CtxRuntime {
  ...
  private async execute(parsed: ParsedCommand): Promise<unknown> {
    ...
    switch (parsed.op) {
      ...
      case "reindex":
        return this.execCloudReindex(parsed);
      case "search":
  ...
  private pickFsKind(parsed: ParsedCommand): FsKind {
    ...
    if (["register-workspace", "import-tree", "reindex", "search", "glob", "grep", "rg"].includes(parsed.op)) {
      return "cloud";
    }
  ...
  private async execCloudReindex(parsed: ParsedCommand): Promise<unknown> {
    const scopeUri = this.optionValue(parsed, "scope-uri") || this.position(parsed, 0) || null;
    const userId = this.resolveCloudUserId(parsed, scopeUri || undefined);
    return this.cloudRequest("POST", "/v1/fs/reindex", {
      userId,
      scopeUri,
    });
  }
